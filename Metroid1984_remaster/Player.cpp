...
Player::Player(float x, float y, float rotation, float speed, float maxSpeed, CollisionManager* collisionManager,SpriteManager* spritemanager) :
...
bool Player::Initialize(LPDIRECT3DDEVICE9 device)
{
	...
	if (!Right_move)
	{
		...
		if (!Right_move->initialize(device, tex, RIGHT_MOVE_INDEX, RUN_COUNT))
		{
			return false;
		}
	}
	...
	if (!Left_move)
	{
		...
		if (!Left_move->initialize(device, tex, LEFT_MOVE_INDEX, RUN_COUNT))
		{
			return false;
		}
	}
	...
	if (!Right_jump)
	{
		...
		if (!Right_jump->initialize(device, tex, RIGHT_JUMP_INDEX, JUMP_COUNT))
		{
			return false;
		}
	}
	if (!Left_jump)
	{
		...
		if (!Left_jump->initialize(device, tex, LEFT_JUMP_INDEX, JUMP_COUNT))
		{
			return false;
		}
	}
	if (!Right_jump_spin)
	{
		...
		if (!Right_jump_spin->initialize(device, tex, RIGHT_JUMP_SPIN_INDEX, JUMP_SPIN_COUNT))
		{
			return false;
		}
	}
	if (!Left_jump_spin)
	{
		...
		if (!Left_jump_spin->initialize(device, tex, LEFT_JUMP_SPIN_INDEX, JUMP_SPIN_COUNT))
		{
			return false;
		}
	}
	if (!Right_ground_spin)
	{
		...
		if (!Right_ground_spin->initialize(device, tex, RIGHT_GROUND_SPIN_INDEX, GROUND_SPIN_COUNT))
		{
			return false;
		}
	}
	if (!Left_ground_spin)
	{
		...
		if (!Left_ground_spin->initialize(device, tex, LEFT_GROUND_SPIN_INDEX, GROUND_SPIN_COUNT))
		{
			return false;
		}
	}
	if (!Right_stand)
	{
		...
		if (!Right_stand->initialize(device, tex, RIGHT_STAND_INDEX, STAND_COUNT))
		{
			return false;
		}
	}
	...
	if (!Left_stand)
	{
		...
		if (!Left_stand->initialize(device, tex, LEFT_STAND_INDEX, STAND_COUNT))
		{
			return false;
		}
	}
...
void Player::Update(float gameTime)
{
	if (status == ObjectStatus::Active)
	{
		...
		{
			if ((Is_jump == true || Is_air == true) && Is_ground_spin == false && !(directionY == 1 && directionX != 0))
			{
			...
			else if (Is_fall == true && directionX != 0)
			{
		...
		_collisionManager->SetLimitation(tempx, tempy);
